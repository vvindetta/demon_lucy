...
def test_pid_is_alive_rejects_non_positive_pids() -> None:
    assert git_state.pid_is_alive(0, runtime_platform="posix") is False
    assert git_state.pid_is_alive(-1, runtime_platform="windows") is False


def test_pid_is_alive_detects_current_process() -> None:
    assert git_state.pid_is_alive(os.getpid(), runtime_platform="posix") is True
...
def test_pid_is_alive_uses_windows_api_branch_without_os_kill(monkeypatch) -> None:
    ...
    assert git_state.pid_is_alive(123, runtime_platform="windows") is True
    assert git_state.pid_is_alive(124, runtime_platform="windows") is False
