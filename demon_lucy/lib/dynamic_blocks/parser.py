...
def _parse_params(
    ...
    *,
    ...
    raise ValueError(
        f"line {block_line}: missing blank line after block parameters"
    )
...
def _find_body(
    ...
    *,
    ...
    if index == start or _without_newline(lines[index - 1]):
        raise ValueError(
            f"line {index + 1}: expected a blank line before end marker"
        )
...
def parse_dynamic_blocks(text: str) -> list[DynamicBlock]:
    ...
    while index < len(lines):
        ...
        if unmatched_end is not None:
            raise ValueError(
                f"line {index + 1}: end marker without matching begin"
            )
